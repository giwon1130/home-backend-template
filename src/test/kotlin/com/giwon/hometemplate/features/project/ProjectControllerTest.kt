...
class ProjectControllerTest {
    ...
    fun `projects endpoint returns public project list`() {
        mockMvc.perform(get("/api/projects"))
            ...
            .andExpect(jsonPath("$.data[8].headline").value("한·미 시장을 차트·수급·뉴스 군집화와 합성 위험도(PizzINT+VIX+뉴스 종합 1~10)로 읽는 개인용 시장 인텔리전스"))
            .andExpect(jsonPath("$.data[9].name").value("BabyLog"))
            .andExpect(jsonPath("$.data[9].repositoryUrl").value("https://github.com/giwon1130/baby-log-api"))
            .andExpect(jsonPath("$.data[10].name").value("Codie"))
            .andExpect(jsonPath("$.data[10].repositoryUrl").value("https://github.com/giwon1130/codie-api"))
